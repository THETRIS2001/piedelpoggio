...
// Interfaccia per i dati di temperatura da Open-Meteo
...
const WeatherWidget: React.FC<WeatherWidgetProps> = ({ className = '' }) => {
  ...
  // Fetch temperature da Open-Meteo (gratuito, senza API key)
  const fetchOpenMeteoData = async (): Promise<OpenMeteoData> => {
    const response = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${LATITUDE}&longitude=${LONGITUDE}&current=temperature_2m&hourly=temperature_2m&daily=temperature_2m_max,temperature_2m_min&timezone=Europe/Rome&forecast_days=5`
    );
    if (!response.ok) {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        // Fetch Open-Meteo per tutte le temperature (prioritario)
        const openMeteo = await fetchOpenMeteoData();
        setOpenMeteoData(openMeteo);

        // Fetch Google Weather per icone, descrizioni, precipitazioni
        ...
        // Genera dati orari da Open-Meteo per temperature e da Google per precipitazioni
        const now = new Date();
        const currentTime = now.getTime();
        ...
        setHourlyData({
          temperatureData,
          precipitationData: hourlyPrecipResult.precipitationData
        });
  ...
  if (error || !weather || !openMeteoData) {
    console.log('WeatherWidget: Rendering stato errore', { error, weather, openMeteoData });
    return (
      <div className={`w-full ${className}`}>

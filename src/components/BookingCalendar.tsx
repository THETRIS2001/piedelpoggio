          ...
          {occupiedForDate.length === 0 ? (
            ...
            <ul className="space-y-2">
              {occupiedForDate.map((b, i) => {
                ...
                return (
                  <li key={i} className="p-3 bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex flex-col gap-2 min-w-0 w-full sm:w-auto">
                      <div className="flex flex-wrap items-center gap-2">
                        {/* Badge Orario */}
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 text-sm font-semibold border border-indigo-100">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          <span>{removeSecondsFromTime(b.start)} – {removeSecondsFromTime(b.end)}</span>
                        </div>

                        {/* Badge Utente */}
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-sm font-semibold border border-amber-100">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                          <span className="truncate max-w-[150px]">{masked}</span>
                        </div>
                      </div>

                      {cleanedTitle && (
                        <div className="flex items-start gap-2 pl-1">
                          <svg className="w-4 h-4 text-emerald-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                          <span className="text-sm text-gray-600 font-medium leading-tight">{cleanedTitle}</span>
                        </div>
                      )}
                    </div>

                    <div className="flex items-center sm:ml-auto pl-1 sm:pl-0">
                      <button
                        onClick={() => handleCancelBooking(b.id)}
                        disabled={isLoading}
                        className="group flex items-center justify-center w-8 h-8 rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden transform transition-all">
            ...
            <div className="bg-red-50 border-b border-red-100 p-6 flex flex-col items-center text-center">
              <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center text-red-600 mb-3 shadow-sm">
              ...
              <p className="text-sm text-gray-600 mt-1 max-w-xs mx-auto">
                Stai per rimuovere definitivamente la prenotazione di <strong className="text-gray-900">{maskCustomerName(showCancelDialog.booking.customer_name)}</strong>
              </p>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-white border border-gray-200 flex-shrink-0 flex items-center justify-center text-gray-500">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-medium">Data</p>
                      <p className="text-sm font-semibold text-gray-900 capitalize">
                        {new Date(showCancelDialog.booking.date).toLocaleDateString('it-IT', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 font-medium">Orario</p>
                      <p className="text-sm font-semibold text-gray-900">
                        {removeSecondsFromTime(showCancelDialog.booking.start)} - {removeSecondsFromTime(showCancelDialog.booking.end)}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mb-6">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`block w-full pl-10 pr-3 py-2.5 border rounded-lg focus:outline-none focus:ring-2 transition-colors ${cancelError
                      ? 'border-red-300 focus:ring-red-500 placeholder-red-300'
                      : 'border-gray-300 focus:ring-red-500'
                      }`}
                    placeholder="Inserisci il numero usato per prenotare"
                    autoFocus
                  />
                </div>
                {cancelError ? (
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    Per sicurezza, inserisci il numero di telefono associato alla prenotazione.
                  </p>
                )}
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200 transition-all"
                >
                  Annulla
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none transition-all"
                >
                  {isLoading ? 'Cancellazione...' : 'Conferma'}
                </button>
              </div>
            </div>
          </div>
